...
/**
 * This is a "Strong Value Type" class for GPIO. The GPIO pin number is checked during construction
 * according to the hardware capabilities. This means that any GPIONumBase object is guaranteed to
 * contain a valid GPIO number. See also the template class \c StrongValue.
 */
...
class GPIONumBase final : public StrongValueComparable<uint32_t> {
   public:
     ...
     *
     * @throw GPIOException if the number does not reflect a valid GPIO number on the current
     * hardware.
     */
 ...
 */
enum class GPIOLevel { HIGH, LOW };

/**
 * Represents a valid pull up configuration for GPIOs.
 * It is supposed to resemble an enum type, hence it has static creation methods and a private
 * constructor. This class is a "Strong Value Type", see also the template class \c StrongValue for
 * more properties.
 */
class GPIOPullMode final : public StrongValueComparable<uint32_t> {
   private:
     ...
     *
     * @param pull_mode A valid numerical respresentation of the pull up configuration. Must be
     * valid!
     */
 ...
 *
 * This class is a "Strong Value Type", see also the template class \c StrongValue for more
 * properties. It is supposed to resemble an enum type, hence it has static creation methods and a
 * private constructor. For a detailed mapping of interrupt types to numeric values, please refer to
 * the driver types and implementation.
 */
class GPIOWakeupIntrType final : public StrongValueComparable<uint32_t> {
   private:
     ...
     * Constructor is private since it should only be accessed by the static creation methods.
     *
     * @param pull_mode A valid numerical respresentation of a possible interrupt level to wake up.
     * Must be valid!
     */
    explicit GPIOWakeupIntrType(uint32_t interrupt_level)
        : StrongValueComparable<uint32_t>(interrupt_level) {}
 ...
 * This class is a "Strong Value Type", see also the template class \c StrongValue for more
 * properties. For a detailed mapping for values to drive strengths, please refer to the datasheet
 * of the chip you are using. E.g. for ESP32, the values in general are the following:
 *  - WEAK:             5mA
 *  - STRONGER:        10mA
 *  - DEFAULT/MEDIUM:  20mA
 *  - STRONGEST:       40mA
...
class GPIODriveStrength final : public StrongValueComparable<uint32_t> {
   public:
     ...
     *
     * @param strength the numeric value mapping for a particular strength. For possible ranges,
     * look at the static creation functions below.
 ...
 *
 * Some functionality is only for specific configurations (set and get drive strength) but is
 * necessary here to avoid complicating the inheritance hierarchy of the GPIO classes. Child classes
 * implementing any GPIO configuration (output, input, etc.) are meant to intherit from this class
...
class GPIOBase {
   protected:
     ...
     *
     * This constructor will only reset the GPIO but leaves the actual configuration (input, output,
     * etc.) to the sub class.
     *
     ...
     *
     * The gpio pad hold function works in both input and output modes, but must be output-capable
     * gpios. If pad hold enabled: in output mode: the output level of the pad will be force locked
     * and can not be changed. in input mode: the input value read will not change, regardless the
     ...
     *
     * @return The currently configured drive strength. Refer to \c GPIODriveStrength for more
     * details.
     *
     * @throws GPIOException if the underlying driver function fails.
     */
...
/**
 * @brief This class represents a GPIO which is configured as open drain output and input at the
 * same time.
 *
