...
PCA9557::PCA9557(idf::I2CMaster &master, idf::I2CAddress address)
...
void PCA9557::configAsOutput(int io_bit) {
    ...
    std::vector<uint8_t> result = master_.sync_transfer(address_, {CONFIG_REGISTER}, 1);
    ...
    result = master_.sync_transfer(address_, {CONFIG_REGISTER, new_value}, 1);
...
void PCA9557::setBit(int io_bit, bool on) {
    ...
    // Get current state
    std::vector<uint8_t> result = master_.sync_transfer(address_, {CURRENT_VALUES_REGISTER}, 1);
    ...
    result = master_.sync_transfer(address_, {OUTPUT_PORT_REGISTER, new_value}, 1);
